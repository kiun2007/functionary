...
//-------------新增代码---------------------
function onWindowCallBack(options, index, layerValue) {

	try{
		var selectTree = window.document.selectTree

		for(var name in this){
			$(this[name]).val(selectTree[name])
		}
	}catch (e) {
		console.log(e)
		layer.msg('未完成选择', { icon: 1 })
		return
	}
...
function startWindow(url, title, ids={name: '#treeName', id: '#treeId'}, options = {width: 380}) {
	...
	var opts = {
		...
		width: options.width,
		url: url,
		callBack: onWindowCallBack.bind(ids, options)
	};
	$.modal.openOptions(opts);
}

function initAsyncTree(options) {
	var defaults = {
		id: "tree",
		view: {
	...
	$.fn.zTree.init($("#" + options.id), options);
}
