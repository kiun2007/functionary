...
function convertUrl(json, url){
	var item = json != null ? (typeof json == 'string' ? JSON.parse(decodeURI(json)) : json) : null
	if (item){
		console.log(item)
		var funStr = url.replaceAll(/\{(.+?)\}/g, '${item.$1}')
		var func = eval('(item)=> `' + funStr +  '`')
		var urlStr = func(item)
		console.log(urlStr)
		return  urlStr
	}else{
		return url
...
function doSubmit(isTree, index, layero){
	...
	var frameName = null
	for(var name in body.context){
		if(name.startsWith("layui-layer-iframe")){
			frameName = name
		}
	}
	...
	$(`#${this.value}`).val(selection[0][this.selectOpt.formValue])
	$(`#${this.selectOpt.label}`).val(selection[0][this.selectOpt.formLabel])
	...
	$.extend({
		...
		modal: {
			...
			msgError: function(content) {
				$.modal.msg(content||'提交失败', modal_status.FAIL);
			},
			// 成功消息
			msgSuccess: function(content) {
				$.modal.msg(content||'提交成功', modal_status.SUCCESS);
			},
			// 警告消息
			msgWarning: function(content) {
				$.modal.msg(content||'警告', modal_status.WARNING);
			},
			// 弹出提示
		...
		operate: {
			...
			enable: function(id, enable) {
				...
				$.modal.confirm(`确定${enable?'启用':'禁用'}${$.table._option.modalName}信息吗？`, function() {
					var url = $.common.isEmpty(id) ? $.table._option.enableUrl : $.table._option.enableUrl.replace("{id}", id);
					$.operate.get(`${url}?enable=${enable}&id=${id}`);
				});
			},
			// 批量删除信息
			removeAll: function() {
			...
			successCallback: function(result) {
				...
				if (result.code == web_status.SUCCESS) {
					...
					if (parent.$.table._option.type == table_type.bootstrapTable) {
						$.modal.close();
						parent.$.modal.msgSuccess(result.msg||'提交成功');
